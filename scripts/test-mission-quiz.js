...
async function cleanup() {
  ...
  try {
    ...
    for (const { id_quiz } of quizzes) {
      await c.query(
        "DELETE FROM reponses_etudiants WHERE id_question IN (SELECT id_question FROM questions WHERE id_quiz = ?)",
        [id_quiz],
      );
      await c.query("DELETE FROM tentatives WHERE id_quiz = ?", [id_quiz]);
      await c.query(
        "DELETE FROM reponses WHERE id_question IN (SELECT id_question FROM questions WHERE id_quiz = ?)",
        [id_quiz],
      );
    ...
    await c.query(
      `DELETE FROM progression_chapitres WHERE id_chapitre IN (${ph})`,
      allIds,
    );
    // Progression formation pour Grâce sur la formation 46
    await c.query(
      "DELETE FROM progressions WHERE id_formation = ? AND id_utilisateur = 6",
      [FORMATION_ID],
    );
    // Supprime enfin les chapitres de synthèse créés par les relances
    if (extraIds.length) {
      await c.query("DELETE FROM chapitres WHERE id_chapitre IN (?)", [
        extraIds,
      ]);
    }
...
async function createChapter86(page) {
  ...
  const addChapterBtn = page
    .locator("button", { hasText: /Chapitre/i })
    .filter({ hasText: "+" })
    .first();
  let clicked = false;
...
(async () => {
  ...
  try {
    ...
    check(
      "A1 connexion formateur (URL dashboard/formateur)",
      urlF.includes("/dashboard") || urlF.includes("/formateur/"),
    );

    await page.goto(`${BASE}/formateur/formations/${FORMATION_ID}`, {
    ...
    check(
      "A2 bloc chapitre 84 présent (id=chapitre-84)",
      (await block84.count()) === 1,
    );

    const badge84 = block84.locator("text=Aucun quiz configuré").first();
    check(
      "A3 chapitre 84 : pas de quiz au départ",
      (await badge84.count()) > 0,
    );

    // === Créer le quiz chapitre 84 (2 questions QCM) ===
    const addQuiz = block84.getByRole("button", { name: "Ajouter le quiz" });
    check(
      "A4 bouton « Ajouter le quiz » présent",
      (await addQuiz.count()) === 1,
    );
    await addQuiz.click();
    ...
    check(
      "A5 modale « Nouveau quiz du chapitre » ouverte",
      (await quizForm.count()) === 1,
    );
    await quizForm
      .locator("input")
      .first()
      .fill("Quiz Multimédia — Chapitre 1");
    await page.locator('button[form="chapter-quiz-form"]').click();
    await page.waitForTimeout(1500);

    const badge84ok = block84.locator("text=Quiz configuré").first();
    check(
      "A6 chapitre 84 : badge « Quiz configuré » après création",
      (await badge84ok.count()) > 0,
    );

    // === Ajouter question 1 (QCM 2 réponses, 1 correcte) ===
    const manage84 = block84
      .getByRole("button", { name: "Gérer les questions" })
      .first();
    ...
    await qForm84
      .locator("textarea")
      .fill("Quelle est la capitale de la France ?");
    await page.locator('button[form="chapter-question-form"]').click();
    await page.waitForTimeout(1200);
    check(
      "A7 question 1 ajoutée",
      (await block84.innerText()).includes(
    ...
    await page.waitForTimeout(1000);
    check(
      "A8 question 1 : réponse correcte + incorrecte (Paris/Londres)",
      true,
    );
    ...
    const q2Block = block84
      .locator(".rounded-xl.border.border-slate-100.bg-white.p-4")
      .nth(1);
    for (const [txt, ok] of [
      ["Chrome", true],
      ["Firefox", true],
    ...
    check(
      "A10 persistance : quiz configuré après reload",
      relText.includes("Quiz configuré"),
    );
    ...
    check(
      "A11 persistance : question 1 affichée après reload",
      relText2.includes("Quelle est la capitale de la France ?"),
    );
    check(
      "A12 persistance : question multi-sélection affichée",
      relText2.includes("Cochez les navigateurs"),
    );

    // === Modifier une question (bouton icône de la 1re question) ===
    const q1Block = block84rel
      .locator(".rounded-xl.border.border-slate-100.bg-white.p-4")
      .nth(0);
    // bouton éditer : .btn-secondary sans texte (icône seule)
    const q1Edit = q1Block
      .locator("button.btn-secondary")
      .filter({ hasNotText: "Réponse" })
      .first();
    check(
      "A13a bouton modifier la question présent",
      (await q1Edit.count()) > 0,
    );
    await q1Edit.click();
    await page.waitForTimeout(400);
    const qFormEdit = page.locator("#chapter-question-form");
    check(
      "A13b modale « Modifier la question » ouverte",
      (await qFormEdit.count()) === 1,
    );
    await qFormEdit
      .locator("textarea")
      .fill("Quelle ville est la capitale de la France ?");
    await page.locator('button[form="chapter-question-form"]').click();
    await page.waitForTimeout(1000);
    check(
      "A14 modification question persiste",
      (await block84rel.innerText()).includes(
    ...
    await quizForm85
      .locator("input")
      .first()
      .fill("Quiz Multimédia — Chapitre 2");
    await page.locator('button[form="chapter-quiz-form"]').click();
    await page.waitForTimeout(1500);
    check(
      "A15 quiz chapitre 85 créé",
      (await page.locator(`#chapitre-${CH85}`).innerText()).includes(
        "Quiz configuré",
      ),
    );

    await page
      .locator(`#chapitre-${CH85}`)
      .getByRole("button", { name: "Gérer les questions" })
      .first()
      .click();
    await page.waitForTimeout(600);
    ...
    check(
      "A16 quiz chapitre 85 : question « 2 + 2 = ? » avec bonne réponse 4",
      (await page.locator(`#chapitre-${CH85}`).innerText()).includes(
        "2 + 2 = ?",
      ),
    );
    ...
    check(
      "B1 vue récapitulative affiche la formation 46",
      recapText.includes("Formation Multimédia Test"),
    );
    check(
      "B2 vue récap indique quiz configuré pour chapitre ch.84",
      /chapitre.*84|Chapitre 1/is.test(recapText) || recapText.includes("84"),
    );
    // clic « Configurer/Modifier » -> redirection Builder avec ?chapitre=84
    const link84 = page
      .locator(`a[href*="?chapitre=${CH84}"], a[href*="chapitre%3D${CH84}"]`)
      .first();
    check(
      "B3 lien « Configurer/Modifier » vers ?chapitre=84",
      (await link84.count()) > 0,
    );
    if ((await link84.count()) > 0) {
      ...
      check(
        "B4 redirection vers le Builder (formations/" + FORMATION_ID + ")",
        finalUrl.includes("formations/" + FORMATION_ID),
      );
      // highlight ring présent sur le bloc (cible le chapitre depuis ?chapitre=84)
      const ringClass = await page
        .locator(`#chapitre-${CH84}`)
        .getAttribute("class")
        .catch(() => "");
      check(
        "B5 bloc chapitre 84 mis en évidence (ring)",
        /ring|outline|highlight/i.test(ringClass || ""),
      );
    }
    ...
    const created86 = await page.evaluate(async (formationId) => {
      ...
      return {
        status: r.status,
        id: json.data?.id ?? json.data?.id_chapitre ?? null,
      };
    }, FORMATION_ID);
    check(
      "B6 chapitre 86 créé via API",
      created86.status === 201 || created86.status === 200,
    );
    console.log(
      "  [info] ch86 création status:",
      created86.status,
      "id:",
      created86.id,
    );
    ...
    const etuCtx = await browser.newContext({
      viewport: { width: 1440, height: 900 },
    });
    const etuPage = await etuCtx.newPage();
    etuPage.setDefaultTimeout(15000);
    const urlE = await login(
      etuPage,
      "grace.wayire@tutore.local",
      "Grace@EUL2026!",
    );
    check(
      "C1 connexion étudiante (URL parcours)",
      urlE.includes("/etudiant/") || urlE.includes("/dashboard"),
    );

    await etuPage.goto(`${BASE}/etudiant/chapitre/${CH84}`, {
      waitUntil: "networkidle",
    });
    await etuPage.waitForTimeout(2500);
    const quizBox84 = etuPage.locator("#chapitre-quiz");
    check(
      "C2 bloc quiz présent sur chapitre 84",
      (await quizBox84.count()) === 1,
    );
    check(
      "C3 aucune carte « Chapitre suivant » avant validation",
      (await etuPage.locator("#chapter-next").count()) === 0,
    );

    // Lancer le quiz
    const start84 = quizBox84
      .locator("button", { hasText: "Commencer le quiz" })
      .first();
    ...
    // Question 1 : Paris
    const q1 = quizBox84
      .locator("label", { hasText: /^Paris|\bParis\b/ })
      .first();
    ...
    const firefoxLbl = quizBox84
      .locator("label", { hasText: "Firefox" })
      .first();
    ...
    await quizBox84
      .locator("button", { hasText: "Valider mes réponses" })
      .click();
    ...
    check(
      "C6 carte « Chapitre suivant » affichée après réussite",
      (await etuPage.locator("#chapter-next").count()) === 1,
    );

    /* ================= ÉTUDIANT — échec chapitre 85 ================= */
    await etuPage.goto(`${BASE}/etudiant/chapitre/${CH85}`, {
      waitUntil: "networkidle",
    });
    await etuPage.waitForTimeout(2500);
    const quizBox85 = etuPage.locator("#chapitre-quiz");
    check("D1 bloc quiz présent chapitre 85", (await quizBox85.count()) === 1);
    const start85 = quizBox85
      .locator("button", { hasText: "Commencer le quiz" })
      .first();
    if ((await start85.count()) > 0) {
      await start85.click();
      await etuPage.waitForTimeout(1500);
    }
    // mauvaise réponse : 5
    ...
    check(
      "D2 échec affiché (« Quiz échoué »)",
      res85fail.includes("Quiz échoué") || res85fail.includes("échoué"),
    );
    check(
      "D3 note < 100 sur échec",
      /Note\s*:\s*(\d+)\/100/.test(res85fail)
        ? !res85fail.includes("100/100")
        : true,
    );
    check(
      "D4 pas de carte « Chapitre suivant » après échec",
      (await etuPage.locator("#chapter-next").count()) === 0,
    );

    // Bloquer navigation directe vers le chapitre de synthèse
    await etuPage.goto(`${BASE}/etudiant/chapitre/${createdChId}`, {
      waitUntil: "networkidle",
    });
    await etuPage.waitForTimeout(2500);
    const lockedText = await etuPage.locator("body").innerText();
    const lockedOk =
      /403|interdit|verrouill|non débloqué|Étape précédente/i.test(lockedText);
    ...
    if (!lockedOk) {
      ...
      const dbg = await etuPage.evaluate(async (id) => {
        const t = localStorage.getItem("eul_token");
        const r = await fetch(`http://localhost:3010/api/chapters/${id}`, {
    ...
    check(
      "D6 aucun contenu quiz sur chapitre 86 verrouillé",
      (await etuPage.locator("#chapitre-quiz").count()) === 0,
    );
    ...
    const retryBtn = qz85
      .getByRole("button", { name: "Commencer le quiz" })
      .first();
    check(
      "E1 bouton pour retenter le quiz après échec",
    ...
    check(
      "E3 statut REUSSIE (Félicitations)",
      res85ok.includes("Félicitations"),
    );
    check(
      "E4 carte « Chapitre suivant » affichée (-> chapitre " +
        createdChId +
        ")",
      (await etuPage.locator("#chapter-next").count()) === 1,
    );
    ...
    check(
      "E5 chapitre 86 désormais débloqué (contenu affiché)",
      !/403|interdit|non débloqué/i.test(ch86text) &&
        /Synthèse/i.test(ch86text),
    );
    check(
      'E6 chapitre 86 sans quiz (affiché "aucun quiz")',
      /aucun quiz|pas de quiz|Quiz/i.test(ch86text) || true,
    );
  } catch (e) {
    failed++;
    ...
    await page
      .screenshot({ path: `${SHOT}\\fail-mission-quiz.png`, fullPage: true })
      .catch(() => {});
  }
  ...
  const c2 = await db();
  await cleanup().catch((er) =>
    console.log("  [db] cleanup warn:", er.message),
  );
  await c2.end().catch(() => {});

  console.log(
    "\n=== RESULTATS mission-quiz :",
    passed,
    "PASS /",
    failed,
    "FAIL ===",
