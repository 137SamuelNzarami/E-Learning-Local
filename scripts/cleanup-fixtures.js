...
(async () => {
  ...
  const [tids] = await conn.query("SELECT id_tentative FROM tentatives WHERE id_utilisateur=8");
  const ids = tids.map(t => t.id_tentative);

  // Réponses des tentatives de l'utilisateur 8 (FK par tentative)
  const col = rc.map(r => r.Field);
  const fkCol = col.includes('id_tentative') ? 'id_tentative' : (col.includes('tentative_id') ? 'tentative_id' : null);
  if (fkCol && ids.length) {
    await q('reponses_etudiants_u8', `DELETE FROM reponses_etudiants WHERE ${fkCol} IN (?)`, [ids]);
  } else {
  ...
  await q('tentatives_u8', 'DELETE FROM tentatives WHERE id_utilisateur=?', [8]);
  await q('inscriptions_u8', 'DELETE FROM inscriptions WHERE id_utilisateur=?', [8]);
  await q('progression_chapitres_u8', 'DELETE FROM progression_chapitres WHERE id_utilisateur=?', [8]);
  await q('progressions_u8', 'DELETE FROM progressions WHERE id_utilisateur=?', [8]);
  await q('notifications_u8', 'DELETE FROM notifications WHERE id_utilisateur=?', [8]);
  await q('avis_u8', 'DELETE FROM avis WHERE id_utilisateur=?', [8]);
  await q('utilisateur_8', 'DELETE FROM utilisateurs WHERE id_utilisateur=?', [8]);
