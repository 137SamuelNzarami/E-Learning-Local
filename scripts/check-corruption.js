...
const conn = mysql.createConnection({ host: 'localhost', port: 3306, user: 'root', password: '', database: 'elearningdb', charset: 'utf8mb4' });

const tables = ['sections','chapitres','formations','quiz','questions','reponses','categories','avis','progression_chapitres'];

conn.query(`SELECT TABLE_NAME, COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='elearningdb' AND DATA_TYPE IN ('varchar','text','longtext','mediumtext','char') ORDER BY TABLE_NAME, ORDINAL_POSITION`, (e1, rows) => {
