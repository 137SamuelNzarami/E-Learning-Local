...
async function main() {
  ...
  console.log("[dom H1]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await page.keyboard.type("Paragraphe de démonstration du RichTextEditor (E2E).");
  await page.keyboard.press("Enter");
  console.log("[dom para]", (await editor.innerHTML()).replace(/\s+/g, " "));
  ...
  console.log("[dom list]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await page.keyboard.type("Visitez la ressource E2E");
  console.log("[dom visiteze]", (await editor.innerHTML()).replace(/\s+/g, " "));
  ...
  console.log("[dom lien]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await editor.evaluate((el) => el.focus());
  await page.keyboard.press("ArrowRight");
  await page.keyboard.press("End");
  await page.keyboard.press("Enter");
  console.log("[dom apres lien+entrer]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await page.locator('button[title="Image par URL"]').click();
  ...
  console.log("[dom image]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await page.locator('button[title="Vidéo par URL"]').click();
  await page.locator('input[placeholder^="https://.../video.mp4"]').fill("https://www.w3schools.com/html/mov_bbb.mp4");
  await page.locator('button:has-text("Insérer")').first().click();
  console.log("[dom video]", (await editor.innerHTML()).replace(/\s+/g, " "));

  await page.locator('button[title="Document (lien téléchargeable)"]').click();
  ...
  console.log("[dom doc]", (await editor.innerHTML()).replace(/\s+/g, " "));
