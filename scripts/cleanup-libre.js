...
(async () => {
  const c = await mysql.createConnection({ host: 'localhost', port: 3306, user: 'root', password: '', database: 'elearningdb' });
  ...
  console.table(libres.map((l) => ({ id_question: l.id_question, quiz: l.quiz, chapitre: l.id_chapitre, enonce: String(l.enonce).slice(0, 60) })));
  ...
  console.table(aCorriger.map((t) => ({ id_tentative: t.id_tentative, id_quiz: t.id_quiz, id_utilisateur: t.id_utilisateur })));
  ...
  try {
    if (idsLibres.length) {
      const [r1] = await c.query('DELETE FROM reponses_etudiants WHERE id_question IN (?)', [idsLibres]);
      console.log(`supprime ${r1.affectedRows} reponses_etudiants de questions LIBRE`);
      const [r2] = await c.query('DELETE FROM questions WHERE id_question IN (?)', [idsLibres]);
      console.log(`supprime ${r2.affectedRows} questions LIBRE`);
    }
    if (idsACorriger.length) {
      const [r3] = await c.query('DELETE FROM reponses_etudiants WHERE id_tentative IN (?)', [idsACorriger]);
      console.log(`supprime ${r3.affectedRows} reponses_etudiants de tentatives A_CORRIGER`);
      const [r4] = await c.query('DELETE FROM tentatives WHERE id_tentative IN (?)', [idsACorriger]);
